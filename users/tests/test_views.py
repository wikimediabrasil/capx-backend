...
class SavedItemViewSetTestCase(TestCase):
    ...
    def test_list_saved_items(self):
        SavedItem.objects.create(user=self.user, relation='sharer', entity='user', entity_id=1)
        SavedItem.objects.create(user=self.user, relation='learner', entity='org', entity_id=2)
    ...
    def test_retrieve_saved_item(self):
        saved_item = SavedItem.objects.create(user=self.user, relation='sharer', entity='user', entity_id=1)
    ...
    def test_create_saved_item(self):
        data = {'relation': 'sharer', 'entity': 'user', 'entity_id': 1}
        response = self.client.post('/saved_item/', data, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        saved_item = SavedItem.objects.get(user=self.user, relation='sharer', entity='user', entity_id=1)
