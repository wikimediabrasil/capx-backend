...
class PortalViewsTests(TestCase):
    ...
    def test_partner_badge_create_permissions(self):
        ...
        payload = {"name": "X", "picture": "http://x", "partner_id": str(self.partner.id)}
        resp = self.client.post(url, data=payload)
        self.assertEqual(resp.status_code, 403)
    ...
    def test_partner_badge_remove_delete_update(self):
        ...
        upd_resp = self.client.post(update_url, data={"badge_id": str(self.badge.id), "name": "Helper2", "picture": "http://new", "description": "desc"})
